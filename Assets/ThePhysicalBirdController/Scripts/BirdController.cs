...
public class Flying : CrashAwareBirdState
{
    ...
    public override BirdState getNextState()
    {
        ...
        if (ctrlRef.RushInput.Value > 0.1f && ctrlRef.FlapInput.Value > 0.1f)
            return new StationaryFlight(ctrlRef);
        if (ctrlRef.RushInput.Value > 0.1f)
        {
    ...
    protected virtual void calculateNewTorques()
    {
        ...
        float pitchTarget = ctrlRef.PitchAxisInput.Value * ctrlRef.Profile.PitchMaxAngle;
...
public class StationaryFlight : Flying
{
    ...
    public override BirdState getNextState()
    {
        ...
        wantsLeaveState = !(ctrlRef.FlapInput.Value > 0.0f && ctrlRef.RushInput.Value > 0.0f);
        if (wantsLeaveState)
        {
