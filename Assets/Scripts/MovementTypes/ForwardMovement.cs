...
[CreateAssetMenu(menuName = "Movement Type/Forward")]

public class ForwardMovement : MovementType
{
    [Divider("Torque")]
    ...
    [Range(0, 90), Tooltip("The maximum angle based on world forward in which the turtle can turn on its y axis.")]
    ...
    public override void ApplyTorque(Vector2 _input, Rigidbody _rigidbody)
    {
        ...
        float angle = Utils.ConvertAngleToRange(_rigidbody.rotation.eulerAngles.y);
        if (_input.x < 0 && angle >= 45)
        {
            return;
        }
